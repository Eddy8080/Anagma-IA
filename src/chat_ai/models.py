...
class AIConsistencyCorrection(models.Model):
    ...
    """
    message = models.OneToOneField(ChatMessage, on_delete=models.CASCADE, related_name='correction')
    ...
    def __str__(self):
        return f"Correção para Msg #{self.message.id} ({self.message.session.user.username})"
